...
- [x] **Implement Runway provider adapter** (`@runwayml/sdk` v3.19, Gen-4.5 text-to-video + image-to-video via `RUNWAYML_API_SECRET`).
- [x] **CLI**: `pnpm pipeline:generate -- <path/to/manifest.json>` with `--dry-run` and `--force` flags.
- [x] **Idempotent** — skip scenes where the `.mp4` already exists unless `--force`.
- [x] **Poll/wait** — uses SDK `waitForTaskOutput()` with 10-minute timeout; SDK auto-retries on 429/5xx.
- [x] **Error handling** — exit non-zero on any failure so pipeline doesn't stitch bad episodes.
- [x] **Auto-update `production/locations.json`** — after generating a scene, writes the asset path back into the matching location entry and sets `conceptBoard: true`.
- [x] Wire up as `pnpm pipeline:generate` in `package.json`.

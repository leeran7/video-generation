...
export function CharacterDetail({
  ...
  const chipClass =
    "rounded-[2px] border border-[color-mix(in_srgb,var(--accent)_30%,transparent)] bg-[color-mix(in_srgb,var(--accent)_12%,transparent)] px-2.5 py-[3px] text-[11px] font-bold uppercase tracking-[0.1em] text-[var(--accent)]";

  return (
    <article className="overflow-hidden rounded border border-[var(--border)] bg-[var(--panel)]" style={style}>
      ...
      <div className="grid items-stretch border-b border-[var(--border)] [grid-template-columns:1fr] min-[721px]:[grid-template-columns:280px_1fr]">
        ...
        <header className="flex flex-col justify-center px-7 pb-6 pt-7">
          ...
          <h2 className="mb-2 mt-0 text-[clamp(28px,4vw,40px)] font-bold tracking-[0.05em] text-[var(--accent)]">
            {character.codename}
          </h2>
        ...
        <div className="flex flex-col gap-3.5">
          {character.seasonOneArc.map((act) => (
            ...
            >
              ...
              <p className="m-0 text-sm leading-[1.55] text-[var(--text)]">
                {act.description}
              </p>
            </div>
          ))}

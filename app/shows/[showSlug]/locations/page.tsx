      ...
      {areas.map((area) => {
        ...
        return (
          <section key={area} className="mt-9 first-of-type:mt-6">
            <header className="mb-3 flex items-baseline gap-3.5 border-b border-[var(--border)] pb-2">
              <span className="text-[11px] font-bold uppercase tracking-[0.4em] text-[var(--muted)]">
                {list.length}
              </span>
              <h2 className="m-0 text-lg tracking-[0.05em]">{area}</h2>
            </header>
            <ul className="m-0 mb-6 grid list-none gap-4 p-0 [grid-template-columns:repeat(auto-fill,minmax(280px,1fr))]">
              {list.map((loc) => (
                <li
                  key={loc.slug}
                  className="flex flex-col overflow-hidden rounded-[2px] border border-[var(--border)] bg-[color-mix(in_srgb,var(--panel)_50%,transparent)]"
                >
                  <div className="flex aspect-video items-center justify-center border-b border-[var(--border)] bg-[color-mix(in_srgb,var(--panel)_80%,black)]">
                    {loc.conceptBoard ? (
                  ...
                  <div className="flex flex-col gap-2 px-3.5 pb-3.5 pt-3">
                    <div className="flex items-baseline justify-between gap-2">
                      <h3 className="m-0 text-sm text-[var(--text)]">
                        {loc.name}
                      </h3>
                      <span
                        className={`whitespace-nowrap rounded-[2px] border px-1.5 py-0.5 text-[10px] uppercase tracking-[0.16em] ${
                          loc.interior
                            ? "border-[color-mix(in_srgb,var(--text)_30%,var(--border))] text-[var(--text)]"
                    ...
                    <p className="m-0 text-xs leading-[1.5] text-[var(--muted)]">
                      {loc.notes}
                    </p>
                  </div>
                </li>

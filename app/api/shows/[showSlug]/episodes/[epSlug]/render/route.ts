  ...
  try {
    ...
    if (!ep.scriptContent) {
      return NextResponse.json(
        { error: "Episode has no script — write a script before rendering." },
        { status: 400 }
      );
    }
    ...
    if (sceneIds.length > 0) {
      ...
      await db
        .update(scenes)
        .set({ status: "pending", error: null, videoPath: null, updatedAt: sql`now()` })
        .where(and(eq(scenes.episodeId, ep.id), inArray(scenes.id, sceneIds)));
    }

    const [job] = await db
      .insert(jobs)
      .values({ type: "episode.render", status: "pending", episodeId: ep.id })
    ...
    try {
      ...
      console.error("[POST .../render] inngest.send failed:", err);
      await db
        .update(jobs)
        .set({ status: "failed", error: "Failed to enqueue render job" })
        .where(eq(jobs.id, job.id));
      return NextResponse.json(
        { error: "Render could not be queued. Is the background job server running?", jobId: job.id },
        { status: 502 }
      );
    }
